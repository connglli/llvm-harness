...
# Note, this should match the one in llvm-harness-install-deps feature
...
if ! id -u ${LLVM_HARNESS_ADMIN_USERNAME} > /dev/null 2>&1; then
    echo "User ${LLVM_HARNESS_ADMIN_USERNAME} does not exist. The script requires ${LLVM_HARNESS_ADMIN_USERNAME} to be created and dependencies installed. Either you did not install llvm-harness-install-deps feature first, or the llvm-harness-install-deps feature defines a different admin user name."
    exit 1
fi
