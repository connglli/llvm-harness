...
@dataclass
class TokenUsage:
  # Input tokens are the tokens in the prompt sent to the model.
  input_tokens: int = 0
  # Cached tokens are the tokens in the prompt that were retrieved
  # from cache instead of being sent to the model.
  # Note that: cached tokens is part of input tokens.
  cached_tokens: int = 0
  # Output tokens are the tokens in the response generated by the model.
  output_tokens: int = 0

  @property
  def total_tokens(self) -> int:
    """Total tokens is usually the sum of input and output tokens."""
    return self.input_tokens + self.output_tokens


@dataclass
class AgentMeter(TokenUsage):
